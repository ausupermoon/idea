...
(function ($) {
    ...
    // Alignement du Sticker Secondaire
    Drupal.behaviors.sticker = {
        attach: function () {
            ...
            var width_sticker;
            var height_sticker;
            $('.node-model.node-teaser, .node-model.upselling, .media-front, .media-panoplie').each(function() {
                width_sticker = $(this).find('.field-name-field-model-sticker-secondary').outerWidth();
                height_sticker = $(this).find('.field-name-field-model-sticker-secondary').outerHeight();
                // Upselling
                if($(this).hasClass('upselling')){
                    if($(this).hasClass('upselling-default')){
                        width_container = $(this).find('.field-name-field-model-image').width();
                    }
                    else{
                        width_container = $(this).width() - $(this).find('.teaser-infos').outerWidth();
                    }
                    height_container = $(this).height();
                }
                // Media Front
                else if($(this).find('.prod-medias').length){
                    width_container = $(this).width();
                    height_container = $(this).height();
                }
                // Panoplie - Product Teaser
                else{
                    width_container = $(this).find('.field-name-field-model-image').width();
                    height_container = $(this).find('.field-name-field-model-image').height();
                }

                if(width_container > width_sticker){
                    $(this).find('.field-name-field-model-sticker-secondary').css('left', ((width_container - width_sticker) / 2)+'px');
                }
                else{
                    $(this).find('.field-name-field-model-sticker-secondary').css('max-width', (width_container - 20)+'px');
                }
                $(this).find('.field-name-field-model-sticker-secondary').css('top', ((height_container - height_sticker) / 2)+'px');
                $(this).find('.field-name-field-model-sticker-secondary').css('display', 'block');
            });
        }
    }

    Drupal.behaviors.ItemsListPosition = {
        attach: function() {
            $('.node-brick-items.items_list .field-name-field-brick-items > .field-items').each(function(){
                var item = $(this).find('> .field-item');
                item.wrapAll($('<div class="flex-container-items"></div>')).end();
            });
        }
    }

    Drupal.behaviors.TetrisMenuActive = {
        attach: function () {
            $('.node-brick.menu .links .field-item').each(function() {
                if(window.location.pathname === $(this).find('> a')[0].pathname) {
                    $(this).addClass('active');
                }
            });
        }
    ...
    $(document).ready(function() {
        $('.node-advice').each(function() {
            color = $(this).attr('data-color');
            if(color){
                if($(this).hasClass('node-teaser')){
                    $(this).find('.advice').css('background-color', color);
                }
                $(this).find('.field-name-title-field').find('h1, h3 a').first().css('color', color);
            }
        });
    ...
    $(window).load(function(){
        ...
        // Selectyze
        $('#edit-sort-reviews, #edit-sort-by, .commerce-add-to-cart select, .view-commerce-cart-form-custom-kameleon select, .node-type-contact .node-contact select').once().Selectyze({
           theme : 'domyos'
        });
